...
int main()
{
    ...
    do
    {
        ...
        case 8:
        {
            ...
            Persona v = personaMayorPatrimonioPaisValor(*personas);

            double t_val = monitor.detener_tiempo();
            long mem_val = monitor.obtener_memoria() - mem0;

            monitor.registrar("Persona de mayor patrimonio del pais (Valor): ", t_val, mem_val);
            std::cout << "\nPersona de mayor patrimonio del pais (por valor)\n";
            v.mostrar();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_val << " ms, Memoria: " << mem_val << " KB\n";
            ...
            long mem0_ref = monitor.obtener_memoria();
            const Persona &r = personaMayorPatrimonioPaisRef(*personas);

            double t_ref = monitor.detener_tiempo();
            long mem_ref = monitor.obtener_memoria() - mem0_ref;

            monitor.registrar("Persona de mayor patrimonio del pais  (Ref): ", t_ref, mem_ref);
            std::cout << "\nPersona de mayor patrimonio del pais (por Referencia)\n";
            r.mostrar();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_ref << " ms, Memoria: " << mem_ref << " KB\n";
            break;
        ...
        case 9:
        {
            ...
            auto mVal = personaMayorPatrimonioCiudadVal(*personas);

            double t_val = monitor.detener_tiempo();
            long mem_val = monitor.obtener_memoria() - mem0;
            monitor.registrar("Patrimonio por ciudad (valor)", t_val, mem_val);

            std::cout << "\nPersona de mayor patrimonio por ciudad (Valor)\n";
            std::cout << "Ciudades: " << mVal.size() << " | Tiempo: " << t_val << " ms | Memoria: " << mem_val << " KB\n";
            for (auto &kv : mVal)
            {
                std::cout << kv.first << ": ";
            ...
            auto mRef = personaMayorPatrimonioCiudadRef(*personas);

            double t_ref = monitor.detener_tiempo();
            long mem_ref = monitor.obtener_memoria() - mem0;

            monitor.registrar("Patrimonio por ciudad (ref)", t_ref, mem_ref);

            std::cout << "\nPersona de mayor patrimonio por ciudad (Referencia)\n";
            std::cout << "Ciudades: " << mRef.size() << " | Tiempo: " << t_ref << " ms | Memoria: " << mem_ref << " KB\n";
            for (auto &kv : mRef)
            {
                std::cout << kv.first << ": ";
        ...
        case 10:
        {
            ...
            auto mVal = personaMayorPatrimonioGrupoValor(*personas, 'A');

            double t_val = monitor.detener_tiempo();
            long mem_val = monitor.obtener_memoria() - mem0;

            monitor.registrar("Mayor patrimonio del grupo A (Valor): ", t_val, mem_val);
            std::cout << "\nPersona con mayor patrimonio del grupo A (por valor)\n";
            mVal.mostrarResumen();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_val << " ms, Memoria: " << mem_val << " KB\n";
            ...
            const Persona &refA = personaMayorPatrimonioGrupoRef(*personas, 'A');

            double t_ref = monitor.detener_tiempo();
            long mem_ref = monitor.obtener_memoria() - mem0_ref;

            monitor.registrar("Mayor patrimonio del grupo A (Ref): ", t_ref, mem_ref);
            std::cout << "\nPersona con mayor patrimonio del grupo A (por referencia)\n";
            refA.mostrar();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_ref << " ms, Memoria: " << mem_ref << " KB\n";

            // Repetir para grupos B y C
            mVal = personaMayorPatrimonioGrupoValor(*personas, 'B');

            t_val = monitor.detener_tiempo();
            mem_val = monitor.obtener_memoria() - mem0;

            monitor.registrar("Mayor patrimonio del grupo B (Valor): ", t_val, mem_val);
            std::cout << "\nPersona con mayor patrimonio del grupo B (por valor)\n";
            mVal.mostrarResumen();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_val << " ms, Memoria:" << mem_val << " KB\n";

            const Persona &refB = personaMayorPatrimonioGrupoRef(*personas, 'B');

            t_ref = monitor.detener_tiempo();
            mem_ref = monitor.obtener_memoria() - mem0_ref;

            monitor.registrar("Mayor patrimonio del grupo B (Ref): ", t_ref, mem_ref);
            std::cout << "\nPersona con mayor patrimonio del grupo B (por referencia)\n";
            refB.mostrar();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_ref << " ms, Memoria: " << mem_ref << " KB\n";

            mVal = personaMayorPatrimonioGrupoValor(*personas, 'C');

            t_val = monitor.detener_tiempo();
            mem_val = monitor.obtener_memoria() - mem0;

            monitor.registrar("Mayor patrimonio del grupo C (Valor): ", t_val, mem_val);
            std::cout << "\nPersona con mayor patrimonio del grupo C (por valor)\n";
            mVal.mostrarResumen();
            std::cout << "\n";
            std::cout << "Tiempo: " << t_val << " ms, Memoria: " << mem_val << " KB\n";

            const Persona &refC = personaMayorPatrimonioGrupoRef(*personas, 'C');
