...
Persona maxPatrNeto::maxPatrNetoValor(std::vector<Persona> personas)
{
    if (personas.empty())
        throw std::runtime_error("No hay personas disponibles");
    Persona mayor = personas.front();
    double maxNeto = mayor.getPatrimonio() - mayor.getDeudas();
    for (size_t i = 1; i < personas.size(); ++i)
    {
        double neto = personas[i].getPatrimonio() - personas[i].getDeudas();
        if (neto > maxNeto)
        {
            mayor = personas[i];
            maxNeto = neto;
...
Persona maxPatrNeto::maxPatrNetoRef(const std::vector<Persona> &personas)
{
    if (personas.empty())
        throw std::runtime_error("No hay personas disponibles");
